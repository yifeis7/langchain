...
def test_pandas_agent_creation(df: DataFrame) -> None:
    agent = create_xorbits_dataframe_agent(OpenAI(temperature=0), df)
...
def test_data_reading_multiple_slices(df: DataFrame) -> None:
    agent = create_xorbits_dataframe_agent(OpenAI(temperature=0), df)
    assert isinstance(agent, AgentExecutor)
    response = agent.run("how many rows with age larger than 50 and food larger than 30 in df? \
                         Give me a number.")
    result = re.search(rf".*({len(df[(df['age'] > 50) & (df['food'] > 30)])}).*", response)
    assert result is not None
    assert result.group(1) is not None

def test_data_reading_no_df_in_prompt(df: DataFrame) -> None:
    agent = create_xorbits_dataframe_agent(
...
def test_multi_df(df_list: list) -> None:
    agent = create_xorbits_dataframe_agent(OpenAI(temperature=0), df_list, verbose=True)
